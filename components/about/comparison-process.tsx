...
import { COMPARISON_PROCESS } from "@/lib/mock/about";
import { HOW_IT_WORKS_STEPS } from "@/lib/mock/how-it-works";

export function ComparisonProcess() {
  return (
    <section
      aria-labelledby="comparison-process-heading"
      ...
      <Container>
        <div className="mb-10 text-center sm:mb-16">
          <p className="mb-3 font-sans text-[11px] font-semibold tracking-[0.15em] text-white/70 uppercase">
            {COMPARISON_PROCESS.eyebrow}
          </p>
          <h2
            id="comparison-process-heading"
            className="font-display text-3xl leading-tight font-bold text-white sm:text-4xl lg:text-[52px]"
          >
            {COMPARISON_PROCESS.heading}{" "}
            <span className="italic">{COMPARISON_PROCESS.headingAccent}</span>
          </h2>
        </div>
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3 lg:gap-7">
          {HOW_IT_WORKS_STEPS.map((step, index) => {
            ...
            return (
              ...
              >
                <div className="absolute top-4 right-6 font-display text-[80px] leading-none font-bold text-white/8 select-none">
                  {step.num}
                </div>
